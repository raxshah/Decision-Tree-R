...
kfold_greedy_algorithm_minlength <- function(filename,separator,head,classColumNum,method){
        ...
        for (fold in 1:10) {
                ...
                while(T){
                        ...
                        total_int_nodes <- summary_train_model %>% filter(NodeType == "IN" ) %>%  summarise(total_int_nodes = n()) 
                        total_int_nodes <<- total_int_nodes[1,]
                        ...
                        if(!isFirstPruning){
                                ...
                                print(paste("Doing Pruning with previous pessimistic error: ",cost_value))
                        ...
                        #compute minimum discription length
                        cost_tree <- 0
                        cost_data_tree <- 0
                        
                        cost_tree <- (total_int_nodes * log2(length(col_withoutClass))) + (total_leaf_nodes * log2(2))
                        cost_data_tree <- (wrongTrainRecords/nrow(test)) * log2(nrow(test))
                        cost_value_n <<- cost_tree + cost_data_tree
  
                        print(paste("Here Total Leaf: ",total_leaf_nodes))
                        
                        if((cost_value_n < cost_value) & (i3 < 3)){
                                
                                cost_value <- cost_value_n
                                
                                print("Minimum Length discription is improved..So doing pruning agian...")
                        }else{
        ...
        png("output/output_minlength_rocCurve.png", width=4, height=4, units="in", res=300)
        plot(roc_curve)
        dev.off()
        
        png("output/output_minlength_prCurve.png", width=4, height=4, units="in", res=300)
        plot(pr_curve)
        dev.off()
        
        out_df1 <- data.frame(Accuracy=accuracy,Balanced_Accuracy = b_accuracy,F1measure= f_measure)
        write.table(out_df1,file= "output/output_minlength_summary.csv",row.names = F,sep = ",")
