...
greedy_algorithm <- function(filename,separator,head = F,classColumNum,method){
 
        set.seed(1)
        
        uci.data <- read.table(file=filename,sep = separator,header = head,stringsAsFactors = F)
